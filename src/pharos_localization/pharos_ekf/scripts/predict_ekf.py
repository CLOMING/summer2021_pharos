...
from pharos_msgs.msg import EBIMUekf_stamped as RPYrate
...
class RosMclEkf:
    ...
    mcl_ekf_odom_pub = rospy.Publisher('/odom/ekf', Odometry, queue_size=1)
    ...
    def ekf_reset(self):
        ...
        self.Q = np.mat(np.diag([0.5, 0.5, 0.005, 0.01]))
    ...
    def mcl_odom_callback(self, msg):
        ...
        if not self.filter_initialized:
            ...
            print("pre dist: ", sqrt(pow(predict_vect[0],2)+pow(predict_vect[1],2)))
            print("pre time: ", predict_time.to_sec())
            x = predict_vect[0]
            y = predict_vect[1]
            ...
            print("use imu: ", self.use_imu_)
            print("use bag: ", self.time_state)
            if self.time_state:
    ...
    def rpy_rate_callback(self, msg):
        self.yaw_rate = msg.ebimu_ekf.YawRate
    ...
    def vehicle_state_callback(self, msg):
        ...
        if msg.GWAY3.GearSelDisp == 2 and abs(self.new_gps[2, 0] - self.old_gps[2, 0]) > 2.0:
            self.back = True
        elif msg.GWAY3.GearSelDisp == 4 and abs(self.new_gps[2, 0] - self.old_gps[2, 0]) > 2.0:
            self.back = False
...
if __name__ == '__main__':
    ...
    rospy.Subscriber('/ebimu/ekf', RPYrate, mcl_ekf.rpy_rate_callback, queue_size=1)
