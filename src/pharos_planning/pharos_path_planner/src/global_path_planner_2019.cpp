...
//        keyboard_position_pub_.publish(keyboard_pose_);
    ...
    /*void CallRequestList_DATA_Callback(const wave_tcp::CallRequestList_DATAConstPtr &msg)  // For Mission State Road graph build
    {
        std::cout << "!!!!!!!!"  << graph_build_<< std::endl;
            ...
            if(NumOfAvailCalls!=0) {
                for (int i = 0; i < NumOfCalls; i++) {
                    if (msg->payload.Call_Data[i].Call_status == 0) {
                        ...
                        std::cout << "passenger_x : " << passenger_x << std::endl;
                        std::cout << "passenger_y : " << passenger_y << std::endl;
...
//        for(int i = 0 ; i<Road_Networks_->roads.size(); i++)    //// need to restore if not click workzone
//        {
//            Road_Networks_->roads.at(i).lanes.clear();
//        }
//        Road_Networks_->roads.clear();
...
//            ROS_ERROR("GlobalPathplanning is re running!!!!!\n");
//            ROS_ERROR("workzone bool data :: %d \n" , workzonebool.data);
//            ROS_ERROR("not graph builded !! \n");
...
//			 }
//            printf("source node : %d\n", msg->roads.at(current_road_-1).lanes.at(current_lane_-1).begin_node);
//            printf("end node : %d\n",msg->roads.at(current_road_-1).lanes.at(current_lane_-1).end_node);
//            printf("destination node :: %d \n", destination_node_);

//            ROS_ERROR("current_road : %d \n" , current_road_);
//            ROS_ERROR("current_lane : %d \n" , current_lane_);
...
//            ROS_ERROR("global_path_planning_request == 1 \n");
            tf::Point p_start, p_wp;
            p_start.setValue(start_pose_.pose.position.x, start_pose_.pose.position.y, 0.0);
            std::vector<Waypoint> waypoints;
            std::vector<Waypoint>::iterator wp_iter;
...
//            std::cout << "starting road :: \n" << starting_road << std::endl;
//            std::cout << "finishing road :: \n" << finishing_road << std::endl;
//            std::cout << "starting lane :: \n" << starting_lane << std::endl;
//            std::cout << "finishing lane :: \n" << finishing_lane << std::endl;
//            std::cout << "start_wp_idx :: \n" << start_wp_index << std::endl;
//            std::cout << "finishing wp_idx :: \n" << finish_wp_index << std::endl;



//            ROS_ERROR("start and finish is same \n");
...
//            ROS_ERROR(" normal case \n");
//            ROS_ERROR("normal case source node :: %d \n", source_node_ );
            DijkstraGraphSearch(source_node_,destination_node_,&road_graph_,&solution_nodes);
            std::reverse(solution_nodes.begin(),solution_nodes.end());

            for(int i = 0; i<road_graph_.size(); i++)
            {
                for(int j = 0; j<road_graph_.size(); j++)
                {
                    if(road_graph_[i][j] != 0)
                    {
//                        ROS_ERROR(" i : %d ", i);
//                        ROS_ERROR(" j : %d ", j);
//                        ROS_ERROR("road graph : %lf ", road_graph_[i][j]);
...
//		printf("\n");
        ...
        for(iter_int=solution_nodes.begin();iter_int != solution_nodes.end()-1;++iter_int){
            ...
            begin_node = *iter_int;
            end_node = *(iter_int+1);
//            ROS_ERROR("begin_node :: %d \n" , begin_node);
//            ROS_ERROR("end_node :: %d \n" , end_node);
...
//            ROS_ERROR("begin node : %d \n" , msg->roads.at(road_number-1).lanes.at(lane_number-1).begin_node);
//            ROS_ERROR("end node : %d \n" , msg->roads.at(road_number-1).lanes.at(lane_number-1).end_node);
...
//        ROS_ERROR("wp_idx :: %lf \n" , old_idx);
//        ROS_ERROR("current_road_number  :: %d \n", current_road_number);
//        ROS_ERROR("current_lane_number  :: %d \n", current_lane_number);
...
//        ROS_ERROR("souce node :: %d \n " , source_node_);
//        ROS_ERROR("present node :: %d\n " , present_node_);
//        ROS_ERROR("Mission state :: %d \n" ,MissionStatus_.data);
    }
    ...
    void DijkstraGraphSearch(const unsigned int start_node, const unsigned int finish_node, std::vector<std::vector<double> > *Graph, std::vector<int>* solution_nodes){
        ...
        while(!finish){
            ...
            for(iter_open = neighbor_nodes.begin();iter_open != neighbor_nodes.end(); iter_open++){
                ...
                {
//                    ROS_ERROR("/////////////////start point ///////////////");
//                    ROS_ERROR("current_node : %d " , current_node);
//                    ROS_ERROR("current node 's parent node : %d " , nodes[current_node].parent_node);
//                    ROS_ERROR("iter open cost : %lf " , nodes[*iter_open].cost);
//                    ROS_ERROR("/////////////////end point ///////////////");
...
//                ROS_ERROR("dist :: %lf " , dist );
//                ROS_ERROR("current node :: %d " , current_node);
//                ROS_ERROR("current node cost %lf" , nodes[current_node].cost);
//                ROS_ERROR("current node 's parent_node %d" , nodes[current_node].parent_node);
//                ROS_ERROR("iter open node :: %d " , *iter_open);
//                ROS_ERROR("iter open node cost:: %lf \n" , nodes[*iter_open].cost);
//                ROS_ERROR("iter open node parent_node :: %d \n" , nodes[*iter_open].parent_node);
...
//        ROS_ERROR("path length         :: %lu " , path_length);
//        ROS_ERROR("input index         :: %d " , old_idx);
//        ROS_ERROR("start index         :: %d " , search_start_idx);
//        ROS_ERROR("search finish index :: %d " , search_finish_idx);
