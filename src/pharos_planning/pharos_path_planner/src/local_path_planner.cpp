...
float objectbox_dist_threshold = 15;
float objectbox_dist_ = 1000;
float objectbox_dist_LPF_alpha_ = 1;
...
void PathPlanning3D::MissionStateCallback(const std_msgs::Int32Ptr msg)
{
...
void PathPlanning3D::ObjectboxDistanceCallback(const std_msgs::Float32ConstPtr& msg){

    float max_dist = 50;
    float target_dist = msg->data==1000?max_dist:msg->data;

    static float objectbox_dist_LPF = 30;
    static int object_missing_cnt = 0;

    objectbox_dist_LPF = target_dist * objectbox_dist_LPF_alpha_ + objectbox_dist_LPF * (1-objectbox_dist_LPF_alpha_);

    objectbox_dist_ = objectbox_dist_LPF;
    std::cout<<"objectbox_dist: "<<objectbox_dist_<<std::endl;
}
...
void PathPlanning3D::ObstacleMapCallback(const nav_msgs::OccupancyGridConstPtr &msg){
    ...
    if(!vehicle_odom_get_ || !vehicle_speed_get_)
    {
        return;
    }
...
//    ROS_ERROR("size of ref_roadinfo size :: %lu \n " , ref_roadinfo_->roadinfo.size());
...
//        ROS_ERROR("wp_pose.x :: %lf \n" , wp_pose.pose.position.x);
//        ROS_ERROR("wp_pose.y :: %lf \n" , wp_pose.pose.position.y);
...
//                ROS_ERROR("roadnumber same but lane number different\n");
...
//    printf("Remain time of current road: %.3f\n", remain_time);
    printf("waypoint index : %d\n", wp_idx);
    printf("current road number : %d\n", ref_roadinfo_->roadinfo.at(wp_idx).road_number);
    printf("current lane number  %d\n", ref_roadinfo_->roadinfo.at(wp_idx).lane_number);



    //    printf("current wp_idx : %d\n",wp_idx);
//    ROS_ERROR("current Mission_State ::  %d " , Mission_states_);
...
//    ROS_ERROR("Ref_path_info : %lu \n" , Ref_path_info_.waypoint.poses.size());
    ...
    lateral_avoiding_result = LateralAvoiding.LateralAvoidingPlanning(LateralAvoiding.reference_path_,
                                                            ...
                                                            obstacle_map,
                                                            EgoVehiclePose,
                                                            init_current_wp_idx, //// init_current_wp_idx
                                                            init_vehicle_speed,
                                                            planning_mode,
                                                            old_path_id,
    ...
    if(lateral_avoiding_result == -1){
        for(unsigned int i=0;i<LateralAvoiding_Ref_Path.speed.poses.size();i++){
            ...
            printf("lateralAvoiding fail because cannot slow down\n");
        }
    }
        ...
        if(DrivingState_.action != 2 && vehicle_speed_ < 0.001){  // traffic light is red 
            stopping_time += dt;
    ...
    if(ForceAvoiding_){
        ...
        force_avoiding_result = LateralAvoiding.LateralAvoidingPlanning(LateralAvoiding.reference_path_,
                                                            ...
                                                            obstacle_map,
                                                            EgoVehiclePose,
                                                            init_current_wp_idx, //// original : init_current_wp_idx
                                                            init_vehicle_speed,
                                                            planning_mode,
                                                            old_path_id,
                                                            lateral_avoiding_path,
...
//    ROS_ERROR("//// CurrentLane laneid -1 :: %d \n" , CurrentRoad.lanes.at(current_lane_id-1).lane_number);
...
//    printf(" CurrentLane road number:: %d     ExitLane road number:: %d \n" , CurrentLane->road_number , ExitLane->road_number);
//    printf(" CurrentLane lane number:: %d     ExitLane lane number:: %d \n" , CurrentLane->lane_number , ExitLane->lane_number);
...
//    printf("middle CurrentLane road number:: %d     ExitLane road number:: %d \n" , CurrentLane->road_number , ExitLane->road_number);
//    printf("middle CurrentLane lane number:: %d     ExitLane lane number:: %d \n" , CurrentLane->lane_number , ExitLane->lane_number);

    unsigned int lane_idx = idx_of_current_lane;
    while(current_road_id == ReferenceLanes_->lanes.at(lane_idx).road_number){

        if(DrivingState_.situation == 70 && Mission_states_ == 6)
        {
//            *ExitLane = ReferenceLanes_->lanes.at(lane_idx+1);
            *ExitLane = CurrentRoad.lanes.at(current_lane_id-1);
        }
//        else{
...
//    printf("after CurrentLane road number:: %d     ExitLane road number:: %d \n" , CurrentLane->road_number , ExitLane->road_number);
//    printf("after CurrentLane lane number:: %d     ExitLane lane number:: %d \n" , CurrentLane->lane_number , ExitLane->lane_number);
...
//        ROS_ERROR("CurrentLane number :: %d \n" , CurrentLane->lane_number);
//        ROS_ERROR("ExitLane number :: %d \n" , ExitLane->lane_number);
        ...
        if(CurrentLane->lane_number != ExitLane->lane_number){
            ...
            if(abs(CurrentLane->lane_number - ExitLane->lane_number)>1){   // lane_changing twice
            ...
            // lane_changing once  this is ordinary situation
            else{
                std::vector<pharos_road_information::Waypoint>::iterator iter;
                for(iter = ExitLane->waypoints.begin(); iter!=ExitLane->waypoints.end();++iter){
                    ...
                    side_lane.curvature.poses.push_back(curv_pose);
                    side_lane.speed.poses.push_back(speed_pose);
//                    for(int i = 0; i<side_lane.speed.poses.size(); i++)
//                    {
//                        ROS_ERROR("final local path speed : %lf " , side_lane.speed.poses.at(i).pose.position.z);
//                          ROS_ERROR("CurrentRoad speed limit : %lf "  , CurrentRoad.speed_limit/3.6);
//                    }
                }
            }
...
//                ROS_ERROR("lane chaning result is 0 !!!");
        ...
        //// for msision5 lane changing the emergency vehicle

        // Lane Changing Case 2

//         if(DrivingState_.situation == 70 && Mission_states_ == 6){
//             ROS_ERROR("emergency_mission_is ended!!!!!!!\n\n\n\n\n\n\n");
//         // if(lateral_avoiding_result != 1 && vehicle_speed_*3.6 < current_road_info.reference_speed*0.6){
//             if(remain_distance > 5.0){
//                 printf("emergency_vehicle is approaching!! , Lane changing\n");
//                 Lateral_Avoiding::Reference_Path side_lane;
//
//                 unsigned int desired_side_lane_idx = 0;
//                 unsigned int desired_side_lane_number = 0;
...
//                                                                                         accumulated_map,
//                                                                                         Mission_states_);
//                 lane_changing_planning_result = 1;
//
//                 if(lane_changing_planning_result == 0){
//                     ROS_ERROR("lane chaning result is 0 !!!");
//                 }
...
//         }
    ...
    if(lane_changing_planning_result != 0){
//        ROS_ERROR("lane_changing_planning_result :: %d \n", lane_changing_planning_result);
        ...
        else if(CurrentLane->lane_number == 3)
            SideBack = LaneChangeState_->left;

        SideBack = true;
        if(lane_change_state_ == 0){
            if(lane_changing_planning_result == 1){
                lane_change_state_ = LANE_CHANGING_READY;  // ready : 1
            }
            ...
            if(lane_changing_planning_result == 1){
                ...
                if(lane_changing_ready_count > 5){
                    lane_change_state_ = LANE_CHANGING_START;  // start : 2
...
//                ROS_ERROR("CurrentLane :: %d ",CurrentLane->lane_number);
//                ROS_ERROR("ExitLane :: %d " , ExitLane->lane_number);
...
//                ROS_ERROR("lane_approach !!!\n ");
//                printf("lane_approach: %.3f\n", lane_approach_);

                if(lane_changing_planning_result == 1){
                    FinalLocalPath = LaneChanging_Ref_Path;
                        
                    if(lane_approach_ < 0.5){  // 0.5 this value has to be valuable
                        lane_change_state_ = LANE_CHANGING_FINISH;  // Lane changing finish : 3
                        std_msgs::Int32 global_path_planning_request_msg;
                        global_path_planning_request_msg.data = 1;
                        global_path_planning_request_pub_.publish(global_path_planning_request_msg);
...
//                        DrivingState_.situation = 70;
                    ...
                    if(lane_approach_ < 0.5){
                        ...
                        if(lane_approach_ < 0.5){
                            lane_change_state_ = LANE_CHANGING_FINISH;
//                            ROS_ERROR("we are in LaneChangeFinish!!!!@!@!@!\n");
                            std_msgs::Int32 global_path_planning_request_msg;
                            global_path_planning_request_msg.data = 1;
//                            ROS_ERROR("Driving situation in finish :: %d \n" , DrivingState_.situation);
                            global_path_planning_request_pub_.publish(global_path_planning_request_msg);
                        }
                    }
                }
...
//            ROS_ERROR("we are in LaneChangeFinish!!!!@!@!@!\n");
//            ROS_ERROR("lane change state :: %d ", lane_change_state_);
    ...
    std_msgs::Bool a_star_algorithm_;

    if( lateral_avoiding_result == 5 /*5*/ && Mission_states_ == 4)
    {
//        FinalLocalPath.waypoint.poses.clear();
//        FinalLocalPath.speed.poses.clear();
//        FinalLocalPath.curvature.poses.clear();
//        ROS_ERROR("we need a* !!! \n");
        DrivingState_.situation = 44;

        a_star_algorithm_.data = true;
        a_star_algorithm_pub_.publish(a_star_algorithm_);

        nav_msgs::PathPtr hybrid_a_star_path(new nav_msgs::Path);
        nav_msgs::PathPtr hybrid_a_star_path_splined(new nav_msgs::Path);

        geometry_msgs::PoseStampedPtr init_vehicle_pose3(new geometry_msgs::PoseStamped);

        Lateral_Avoiding::Reference_Path ref_a_star_path;

        *init_vehicle_pose3 = init_vehicle_pose;

//        std::cout << "local_path_planner ref_path.roadinfo.size ::  "<<ref_path.roadinfo.size()  << std::endl;

//        ROS_ERROR("in local path planner / referenecpath size :: %d \n",LateralAvoiding.reference_path_->waypoint.poses.size());

        ref_goal_speed_ = 10;

        Hybrid_A_Star.HybridAStarPathPlanning(*LateralAvoiding.reference_path_,
                                              CurrentLane,
                                              ref_roadinfo_,
                                              init_vehicle_pose3,
                                              EgoVehiclePose,
        ...
        if(hybrid_a_star_path->poses.empty() != 1)
        {
            B_spline(hybrid_a_star_path, hybrid_a_star_path_splined);
            for(int i = 0 ; i<hybrid_a_star_path_splined->poses.size() ; i++)
            {
                hybrid_a_star_path_splined->poses.at(i).pose.position.z = ref_goal_speed_;

//                ROS_ERROR("hybrid a star speed :: %lf \n\n\n" , hybrid_a_star_path_splined->poses.at(i).pose.position.z);
            }
        }else
        {
            ROS_ERROR("hybrid a star path size : 0\n");
        }


        ref_a_star_path.waypoint.poses = hybrid_a_star_path_splined->poses;
        ref_a_star_path.speed.poses = hybrid_a_star_path_splined->poses;
        ref_a_star_path.curvature.poses = hybrid_a_star_path_splined->poses;

//        std::cout << "not splined path \n " << *hybrid_a_star_path << std::endl;
//        std::cout << " splined path \n" << ref_a_star_path.speed << std::endl;

        if(ref_a_star_path.speed.poses.empty() != 1)
        {
//            ROS_ERROR("final local path size :: %lu \n" , FinalLocalPath.waypoint.poses.size());
            FinalLocalPath = ref_a_star_path;   //planner_solution_path is fianl path
        }
//        ROS_ERROR("Out side of Hybrid A star !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! \n");
        ...
        a_star_algorithm_.data = false;
        a_star_algorithm_pub_.publish(a_star_algorithm_);
    }

//    printf("Lane change state: %d\n",lane_change_state_);
    printf("Driving_state_situation :: %d \n",DrivingState_.situation );
//    ROS_ERROR("Mission_state           :: %d \n",Mission_states_ );
//    ROS_ERROR("a_star_algorithm_bool :: %d \n",a_star_algorithm_.data);
    FinalLocalPath.waypoint.header.frame_id = "odom";
    FinalLocalPath.waypoint.header.stamp = obstacle_map->header.stamp;
    FinalLocalPath.speed.header.frame_id = "odom";
    FinalLocalPath.speed.header.stamp = obstacle_map->header.stamp;
    ...
    if(lateral_avoiding_result == -2 && Mission_states_ == 1)
    {
        Emergency_state_.data = true;
    }

//    ROS_ERROR("vehicle_speed :: %lf \n " , vehicle_speed_);
//     if(vehicle_speed_ < 0.01 && Emergency_state_.data == true && (lateral_avoiding_result == -3 || lateral_avoiding_result == 1)){
//         go_count ++;
// //        ROS_ERROR("go count :: %lf \n " , go_count );
//     }
    // if(go_count > 5)
    // {
    //     Emergency_state_.data = false;
    //     go_count = 0;
    // }

    /* YH: emergency state 2022 */
    // if (current_road_info.road_number == 2 || current_road_info.road_number == 9){
    static ros::Time ros_time = ros::Time::now();
    ros::Duration ros_dt = ros::Time::now() - ros_time;
    ros_time = ros::Time::now();
    static float go_count = 0;

    if (true){
        if (objectbox_dist_ < objectbox_dist_threshold){
            Emergency_state_.data = true;
            go_count = 0;
        }
    }
...
//    ROS_ERROR("Final local path size :: %lu \n ", FinalLocalPath.waypoint.poses.size());
    Emergency_state_pub.publish(Emergency_state_);
...
int PathPlanning3D::init(){
    ...
    pnode_->param<float>("objectbox_dist_threshold", objectbox_dist_threshold, 15);
    pnode_->param<float>("lpf_alpha", objectbox_dist_LPF_alpha_, 1);
    ...
    vehicle_odom_sub_ = node_->subscribe("/odom/vehicle",10,&PathPlanning3D::VehicleOdomCallback,this);
    ...
    mission_state_sub_ = node_->subscribe("mission_state",10,&PathPlanning3D::MissionStateCallback,this);

    dist_sub_ = node_->subscribe("objectbox_dist",10,&PathPlanning3D::ObjectboxDistanceCallback,this);
    ...
    a_star_algorithm_pub_ = node_->advertise<std_msgs::Bool>("a_star_algorithm_state",10);
    Driving_state_pub_ = node_->advertise<pharos_behavior_planner::DrivingState>("/local/driving_state",10);
    Emergency_state_pub = node_->advertise<std_msgs::Bool>("emergency_state",10);
...
int main(int argc, char **argv)
{
    ...
    if (Hybrid_A_Star.init())
    {
        ROS_FATAL("3d_path_planner_node [hybrid a star] initialization failed");
        return -1;
    }
