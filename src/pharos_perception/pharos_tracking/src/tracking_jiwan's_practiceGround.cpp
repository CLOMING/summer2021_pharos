    ...
    void tracking_CB(const pharos_msgs::ObjectInfoArrayPtr msg) {
        ...
        for (int i = 0; i < new_Infoarray->objects.size(); i++) {
            ...
            *objectpose = msg->objects[i];
            ...
            objectpose->maintain_count = 0; // 유지 카운트 저장.
            odom_Infoarray->objects.push_back(*objectpose); // 하위 객체에 pushback
        ...
        /*
         * 처음 main함수가 돌 때만 실행된다.
         * 그 의미가 무엇이냐면 전에 있는 것을 기억하기 위해서 현재 있는 것을 odom에 저장함.
         * 전역 변수로 false 로 이미 저장해둠.
         */
        ...
        // std::cout << "2" << std::endl;
            ...
            for (int i = 0; i < old_Infoarray->objects.size(); i++) {
                ...
                /*
                 * velo 를 이용해서 expected Info를 추정함.
                 */
...
//                cout << "expected_Infoarray_type : " <<expected_Infoarray->objects[i].type <<endl;
            ...
            float d_deadline = 3; // 거리 최솟값
            std::vector<std::vector<std::vector<double>>> vec_odom_array;

            std::vector<int> tracked_index_array;
            for (int i = 0; i < odom_Infoarray->objects.size(); i++) {
                double d_min = 10000; // 최솟값을 일단 10000으로 둠
                int tracked_index = 0;
                ...
                double x_max = odom_Infoarray->objects[i].pose.x+2.5;
                double x_min = odom_Infoarray->objects[i].pose.x-2.5;
                double y_max = odom_Infoarray->objects[i].pose.y+2.5;
                double y_min = odom_Infoarray->objects[i].pose.y-2.5; // 박스를 만들어서 그 사이에 있는 건 같은 object라고 인식
                // 아무래도
                ...
                tracked_index_array.push_back(expected_Infoarray->objects[tracked_index].id);


                std::vector<double> vec_d_min;
                ...
                std::vector<std::vector<double>> vec_odom;
            ...
            for (int i=0; i<expected_Infoarray->objects.size();i++) {
                ...
                for (int k = 0; k < tracked_index_array.size(); k++) {
                    if (expected_Infoarray->objects[i].id != tracked_index_array[k]) { not_tracked_index = i; }
                    else {
                        not_tracked_index = -1;
                        break;
                    }
            ...
            for (int k = 0; k < vec_odom_array.size(); k++) {
                for (int l = 0; l < vec_odom_array[k].size(); l++) {
                    std::sort(vec_odom_array.begin(), vec_odom_array.end());
                }
            }
            ...
            for (int k = 0; k < vec_odom_array.size(); k++) {
                ...
                if (vec_odom_array[k][0][0] < d_deadline) {
                    ...
                    if (vec_odom_array[k][13][0] < 9) {
                        ...
                        undefined_x = vec_odom_array[k][5][0]; // 들어온 건 이것 뿐,  벨로다인에서 갑자기 값이 튀어서 값이 들어오지 않을 때 때문에
...
//                }
            ...
            for (int i = 0; i < odom_Infoarray->objects.size(); i++) {
                ...
                odom_Infoarray->objects[vec_odom_array[i][1][0]].id = vec_odom_array[i][2][0];
...
//                odom_Infoarray.objects[vec_odom_array[i][1][0]]. = vec_odom_array[i][15];
                ...
                matching_count_5=0;
                //type을 해당 id에 저장
                if(odom_Infoarray->objects[vec_odom_array[i][1][0]].type==1)
                {   matching_type1_size ++;
                ...
                *undefinded_objectpose = odom_Infoarray->objects[vec_odom_array[i][1][0]];
                ...
                undefinded_objectpose->id = vec_odom_array[i][16][0];
...
//visualization_msgs::Marker marker;
            ...
            {
                if(undefinded_Infoarray->objects[i].pose.x!=10000)
                {
                    *undefinded_objectpose = undefinded_Infoarray->objects[i];
                    added_Infoarray->objects.push_back(*undefinded_objectpose);
                }
                }
        ...
        //added_infoarray 중에 odom_infoarray와 겹치는 것을 제외시켜준다.
            ...
            //real_added_infoarray 중에 last_added_infoarray와 겹치는 것을 제외시켜준다.
...
//            }
            ...
            for(int i=0; i<odom_Infoarray->objects.size(); i++)
            {
                ...
                {
                    double now_velo_x = accumulate(odom_Infoarray->objects[i].x_velo.begin(), odom_Infoarray->objects[i].x_velo.end(), 0.0)/ odom_Infoarray->objects[i].x_velo.size();
...
//                cout << "odom_theta : " <<atan2(odom_Infoarray.objects[i].pose.velo_y,odom_Infoarray.objects[i].pose.velo_x) << endl;
                ...
                if(odom_Infoarray->objects[i].maintain_count >10) {
                    if (fabs(odom_Infoarray->objects[i].collision.min_y
